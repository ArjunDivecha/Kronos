"""
Extended Backtest: UK 531 Stocks (2000-2014)
===========================================
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
DATA_DIR      = "data/UK500"
FORECAST_CSV  = "forecasts_UK500_2000_2014.csv"
LOOKBACK      = 40
PRED_LEN      = 5
STRIDE        = 5
SAMPLE_COUNT  = 5
...
def run_backtest(tickers, predictor):
    ...
    if not stock_dfs:
        print("No UK data found for the 2000-2014 period.")
        return
    ...
    periods_done = 0
    for i in tqdm(all_periods, desc="Backtesting UK (2000-2014)"):
...
if __name__ == "__main__":
    df_meta = pd.read_excel("/Users/arjundivecha/Dropbox/AAA Backup/A Working/Kronos/UK.xlsx")
    raw = df_meta["Ticker"].dropna().tolist()
    tickers = []
    for t in raw:
        t = t.replace(" LN Equity", "").replace(" LN", "").strip()
        t = re.sub(r'[/]+$', '', t).strip()
        t = re.sub(r'\.', '-', t) + ".L"
        tickers.append(t)
