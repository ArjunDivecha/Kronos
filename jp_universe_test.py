"""
Large Universe Test: Japanese Stocks
====================================
Zero-shot Kronos-base backtest on the Japanese market.
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
JP_EXCEL_FILE = "/Users/arjundivecha/Dropbox/AAA Backup/A Working/Kronos/Japan.xlsx"
DATA_DIR      = "data/Japan"
FORECAST_CSV  = "forecasts_Japan.csv"
LOOKBACK      = 40
PRED_LEN      = 5
STRIDE        = 5
SAMPLE_COUNT  = 5
...
# ── LOAD TICKERS ──────────────────────────────────────────────────────────────
def load_jp_tickers():
    df = pd.read_excel(JP_EXCEL_FILE)
    raw = df["Ticker"].dropna().tolist()
    
    cleaned = []
    for t in raw:
        # Expected format: "1332 JT Equity" -> "1332.T"
        t = t.replace(" JT Equity", "").strip()
        t += ".T"
        cleaned.append(t)
    print(f"Loaded {len(cleaned)} Japanese tickers.")
    return cleaned
...
def download_universe(tickers):
    ...
    for ticker in tqdm(to_download, desc="Downloading JP Data"):
    ...
    print(f"  ✅ {len(valid)}/{len(tickers)} valid JP stocks")
    return valid
...
def run_backtest(tickers, predictor):
    ...
    periods_done = 0
    for i in tqdm(all_periods, desc="Backtesting JP"):
...
if __name__ == "__main__":
    ...
    print("\n✅ Japan Test Fully Completed. Outputs are in", FORECAST_CSV)
