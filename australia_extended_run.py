"""
Extended Backtest: Australia 500 Stocks
=======================================
Sources Australia OHLCV history from Bloomberg, then runs the Kronos backtest
over a configurable historical window.

Default flow:
1. Spawn a Bloomberg-only download pass in the shared OpusBloomberg env.
2. Save repo-native CSVs under data/Australia/.
...
AUSTRALIA_EXCEL_FILE = Path("/Users/arjundivecha/Dropbox/AAA Backup/A Working/Kronos/Australia.xlsx")
DATA_DIR = REPO_DIR / "data" / "Australia"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Australia extended Kronos run with Bloomberg data.")
...
def load_australia_universe(limit=None):
    df = pd.read_excel(AUSTRALIA_EXCEL_FILE)
    records = []
    seen = set()
    ...
    print(f"Loaded {len(records)} Australia securities from {AUSTRALIA_EXCEL_FILE.name}.")
    return records
...
def forecast_csv_path(start_date, analysis_end_date, limit=None):
    ...
    base_name = f"forecasts_Australia_{start_date[:4]}_{end_year}"
    if limit is not None:
...
def download_bloomberg_history(records, start_date, end_date, force_refresh=False):
    ...
    with BBG() as bbg:
        ...
        for record in tqdm(records, desc="Downloading Australia Bloomberg history"):
    ...
    print(f"Saved Bloomberg history for {valid_count}/{len(records)} Australia securities.")
...
def load_stock_frames(records, analysis_end_ts):
    stock_dfs = {}
    for record in tqdm(records, desc="Loading Australia data"):
...
def run_backtest(records, predictor, forecast_csv, analysis_end_ts):
    ...
    if not stock_dfs:
        print(f"No Australia data found through {analysis_end_ts.date()}.")
        return pd.DataFrame()

    n_stocks = len(stock_dfs)
    top_n = max(1, int(n_stocks * DECILE))
    print(f"\n  {n_stocks} valid Australia stocks through {analysis_end_ts.date()} | Top/Bottom 10% = {top_n} stocks per bin")
    ...
    periods_done = 0
    for idx in tqdm(all_periods, desc=f"Backtesting Australia ({analysis_end_ts.year})"):
...
def main():
    args = parse_args()
    records = load_australia_universe(limit=args.limit)
    ...
    print("\nAustralia extended run completed.")
